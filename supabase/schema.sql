...
as $$
begin
  update public.shared_plenaries
  set state = p_state,
      updated_at = now()
  where share_token = p_share_token;
...
as $$
begin
  delete from public.shared_plenaries
  where share_token = p_share_token;
...
revoke all on table public.shared_plenaries from anon, authenticated;
